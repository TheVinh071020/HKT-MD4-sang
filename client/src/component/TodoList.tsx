...
const TodoList = (props: Props) => {
  ...
  return (
    <div className="mt-10">
      <div
        style={{ margin: "0 auto" }}
        className="bg-rose-400 p-4 rounded-lg shadow-md w-2/5 container"
      >
        ...
        <div className="mb-4 flex">
          <input
            type="text"
            className="w-full px-2 py-1 border text-gray-700 border-gray-300 rounded-md -opacity-70"
          ...
          {users.length > 0 &&
            users.map((e: any, i: any) => (
              <div className="flex justify-between" key={i}>
                <span className={e.status === 1 ? "line-through" : ""}>
                  {e.name}
                </span>
                <span className="flex gap-5 items-center">
                  <button>
                    <input
                      checked={e.status == 1}
                      onClick={() => handleUpdate(e.todo_id)}
